...
\section{Tokens}
El Analizador Léxico de MLOR es generado gracias a la utilización de la herramienta automática JLex. Dicho analizar reconoce los tokens especificados en la tabla \ref{tab:tokens}, y los pasa \emph{on demand}
\begin{table}[H]
...
\chapter{DDS}
La Definición Dirigida por la Sintaxis se encuentra en el archivo ``dds.pdf''. En ella se encuentra una implementación análoga a la utilizada en el código fuente del intérprete.

\section{Estructuras}
\begin{itemize}
	\item \textbf{\texttt{Attrs}}: El conjunto de atributos de los no terminales. Mantiene el tipo de la subexpresión computada, y el conjunto de identificadores, guardadas en una lista, que referencian a dicho tipo. Además, guarda todo lo necesario para los diferentes tipos (tipo de elemento para las listas, tipo de dominio y rango para las funciones, etc,).
	\item \textbf{Tablas de símbolos}: Se utilizaron dos tablas de símbolos, implementados con \texttt{Hashtables}, con cadenas de caracteres que contienen el identificador como llave, y Attrs como contenido. La primer tabla es la global, que es utilizada en toda la ejecución. La segunda sólo vive durante la ejecución de funciones, y se utiliza como un tabla de símbolos local.
\end{itemize}
...
Por ejemplo, si una variable x es de tipo \texttt{('a -> 'b)}, el objeto \texttt{Attrs} 'a tendrá que saber que es apuntado indirectamente por x, así como también 'b. Para esto, en la lista de identificadores se agrega la variable seguida por un prefijo, que indica por quién está apuntado. En el caso de los dominios de una función, el prefijo es ``-'', mientras que para el rango es ``\textgreater'', por lo que en las listas iran ``-x'' y ``\textgreater x'' respectivamente. Esta información es utilizada luego para corregir las referencias cuando el tipo es cambiado.

Para las listas, se le agrega como prefijo ``['', y para las tuplas ``(i)'', donde \textit{i} es el índice de la referencia en la lista de tipos.

\section{Métodos}
\begin{itemize}
	\item \textbf{tipoMasEspecifico}: Devuelve el tipo más específico entre los dos parámetros, si es que son compatibles. En el caso de los tipos estructurados (listas, tuplas y funciones), se llama recursivamente. Este método también corrige las referencias de las variables, directa e indirectamente. 
	
	Los politipos pueden cambiar de tipo, por lo que se selecciona como tipo más específico el otro, y todos apuntan al segundo. En el caso de que los dos sean politipos, seguirán siéndolo, pero las variables que los apuntaban ahora apuntaran al mismo objeto.
	\item \textbf{esTipoTextOrNum}: Devuelve si el tipo pasado es un \textit{string}, o un número.
	\item \textbf{crearPolitipo}: Crea un objeto \texttt{Attrs} del tipo que se le pasa por parámetro, utilizando también un entero contador que lo representa unívocamente.
	\item \textbf{obtenerTipo}: Busca primero en la tabla de símbolos local (si existe) y luego en la global el tipo del identificador pasado por parámetro.
	\item \textbf{esTipoSimple}: Devuelve si el tipo pasado es un \textit{string}, un número, o un \textsl{boolean}.
\end{itemize}
